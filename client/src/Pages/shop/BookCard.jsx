...
const BookCard = ({ book, onDelete }) => {
  ...
  if (token) {
    try {
      ...
      isAdmin = decoded.role === 'admin'; // ✅ adjust based on your token structure
    } catch (error) {
    ...
    <div className="group shadow-md rounded-lg">
      ...
      <div className="aspect-square mb-4 overflow-hidden rounded-lg bg-yellow-100 p-8 relative group">
        ...
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <NavLink to={`/books/${book._id}`} className="bg-amber-500 text-white py-2 px-4 rounded cursor-pointer">
            View Details
          </NavLink>
        </div>
      </div>
